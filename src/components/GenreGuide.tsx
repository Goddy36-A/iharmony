...
import { Music, Clock, Gauge, Play, Pause, Sparkles, Search, Volume2, ExternalLink } from 'lucide-react';
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="glass rounded-xl p-5">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Every genre with BPM, scales, prompt tips, and royalty-free audio samples. Discover genres you didn't know existed.
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredGenres.map(genre => (
          <div key={genre.name} className="glass rounded-xl p-5 space-y-4 hover:border-primary/30 transition-all">
            ...
            {/* Audio Samples */}
            {genre.sampleLinks && genre.sampleLinks.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-1.5">
                  <Volume2 className="w-3.5 h-3.5 text-accent" />
                  <span className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Listen — Royalty Free Samples</span>
                </div>
                {genre.sampleLinks.map(link => (
                  <button
                    key={link.url}
                    onClick={() => togglePlay(link.url)}
